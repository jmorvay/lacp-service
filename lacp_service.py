...
def configure_service(service_id, json_body):
    ...
    add_port_to_bundle(node1_name, bundle_id, node1_ports)
    ...
    add_port_to_bundle(node1_name, bundle_id, node2_ports)

    uniconfig_api = UniconfigManagerApi(configuration)
    commit_nodes = UniconfigManagerCalculatediffInputTargetnodes([uniconfig_node1, uniconfig_node2])
...
def add_port_to_bundle(node_id, bundle_id, port):
    bundle_ifc_name = 'BundleEther' + bundle_id
    ...
    etc_ifc_api.put_network_topology_network_topology_topology_node_configuration_interfaces_interface_ethernet_config(
        ...
        frinx_openconfig_if_ethernet_ethernettop_ethernet_config_body_param=port_ifc_eth_config
    )
...
def create_bundle(node_id, bundle_id):
    ...
    bundle_ifc_config = FrinxOpenconfigInterfacesInterfacestopInterfacesInterfaceConfig(
        frinx_openconfig_interfacestype=FrinxOpenconfigInterfacesTypeIdentityref.IANA_IF_TYPE_IEEE80211,
